        ...
        for (int y = 0; y < ny; ++y) {
            for (int z = 0; z < nz; ++z) {
                ...
                const float boundary_bg_x = clamp_lattice_speed(
                    forcing[base + kChBackgroundWindX] / std::max(1.0e-6f, transport.velocity_scale_m_s_per_lattice)
                );
                const float boundary_bg_z = clamp_lattice_speed(
                    forcing[base + kChBackgroundWindZ] / std::max(1.0e-6f, transport.velocity_scale_m_s_per_lattice)
                );
